...
const products = [
  { id: 1, name: 'Produk A', price: 20000, image: ''},
  { id: 2, name: 'Produk B', price: 30000, image: ''},
  { id: 3, name: 'Produk C', price: 50000, image: ''    },
];
...
const Card = ({ product, updateTotal }) => {
  ...
  return (
    <div className="border rounded-lg shadow-lg p-4 w-64 m-4 bg-gradient-to-r from-blue-100 via-blue-50 to-blue-100">
      <img src={"https://via.placeholder.com/300x200"} alt={product.name} className="w-full h-40 object-cover rounded-md" />
      ...
      <p className="text-gray-800 mt-4">Total: Rp {totalPrice.toLocaleString()}</p>
    </div>
  );
};
          ...
          {products.map((product) => (
            <li key={product.id} className="flex justify-between py-2 border-b">
              ...
              <span className="text-gray-800">
                {totals[product.id]} x Rp {product.price.toLocaleString()} = Rp {(totals[product.id] * product.price).toLocaleString()}
              </span>
            </li>
          ))}
        </ul>
        <h3 className="text-lg font-bold mt-4 text-gray-800">Total Keseluruhan: Rp {grandTotal.toLocaleString()}</h3>
      </div>
    </div>
  );
};
